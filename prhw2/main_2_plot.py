# TODO: main_2.py but with plotting showing before and after BPoly correction. We have 10 data sets. Maybe do a 2x10 grid of subplots?
# TODO: generate tabular data for unknown datasets
...
for letter in datasets:
    ...
    #----------Plotting----------
    # Plot 3 visualizations: before correction, after correction, and error vectors
    print(f"Generating plots for dataset {letter}...")
    
    # Plot 1: Before correction (measured vs expected)
    fig1, ax1 = plotter.plot_data_error_vectors(
        C_expected_flat, 
        C_measured_flat,
        f"C_expected_{letter}",
        f"C_measured_{letter}"
    )
    ax1.set_title(f"Dataset {letter}: Before Correction\nRMS = {C_rms:.4f} mm")
    plt.savefig(f"./out/pa2-{prefix}-{letter}-before.png", dpi=150, bbox_inches='tight')
    plt.close(fig1)
    
    # Plot 2: After correction (expected vs corrected)
    corrected_rms = calcerr.calculate_rms_error(C_predicted_flat, C_expected_flat)
    fig2, ax2 = plotter.plot_data_error_vectors(
        C_expected_flat,
        C_predicted_flat,
        f"C_expected_{letter}",
        f"C_corrected_{letter}"
    )
    ax2.set_title(f"Dataset {letter}: After Correction\nRMS = {corrected_rms:.4f} mm")
    plt.savefig(f"./out/pa2-{prefix}-{letter}-after.png", dpi=150, bbox_inches='tight')
    plt.close(fig2)
    
    # Plot 3: Error comparison (measured vs corrected overlaid on expected)
    fig3 = plt.figure(figsize=(14, 6))
    
    # Before subplot
    ax3a = fig3.add_subplot(121, projection='3d')
    ax3a.scatter(C_expected_flat[:, 0], C_expected_flat[:, 1], C_expected_flat[:, 2], 
                c='green', alpha=0.5, s=20, label='Expected')
    ax3a.scatter(C_measured_flat[:, 0], C_measured_flat[:, 1], C_measured_flat[:, 2], 
                c='red', alpha=0.5, s=20, label='Measured')
    ax3a.set_xlabel('X')
    ax3a.set_ylabel('Y')
    ax3a.set_zlabel('Z')
    ax3a.set_title(f'Before: RMS = {C_rms:.4f} mm')
    ax3a.legend()
    
    # After subplot
    ax3b = fig3.add_subplot(122, projection='3d')
    ax3b.scatter(C_expected_flat[:, 0], C_expected_flat[:, 1], C_expected_flat[:, 2], 
                c='green', alpha=0.5, s=20, label='Expected')
    ax3b.scatter(C_predicted_flat[:, 0], C_predicted_flat[:, 1], C_predicted_flat[:, 2], 
                c='blue', alpha=0.5, s=20, label='Corrected')
    ax3b.set_xlabel('X')
    ax3b.set_ylabel('Y')
    ax3b.set_zlabel('Z')
    ax3b.set_title(f'After: RMS = {corrected_rms:.4f} mm')
    ax3b.legend()
    
    fig3.suptitle(f'Dataset {letter}: Distortion Correction Comparison', fontsize=14, fontweight='bold')
    plt.savefig(f"./out/pa2-{prefix}-{letter}-comparison.png", dpi=150, bbox_inches='tight')
    plt.close(fig3)
    
    print(f"Plots saved for dataset {letter}")
    print(f"  Before correction RMS: {C_rms:.4f} mm")
    print(f"  After correction RMS:  {corrected_rms:.4f} mm")
    print(f"  Improvement: {C_rms - corrected_rms:.4f} mm\n")
