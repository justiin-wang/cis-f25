...
(OPTIONAL) main_2_plot.py: performs the same thing as main_2 but also generates graphical outputs
...
(OPTIONAL) Instructions to run main_2_plot.py: execute with python version and packages as specified in requirements.txt while in the PROGRAMS directory. Outputs for all files in the data folder (by default contains all files provided for the assignment) will be generated in OUTPUT_graph/ folder.